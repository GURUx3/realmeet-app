...
class SocketService {
        ...
        this.io.on('connection', (socket) => {
            ...
            socket.on('disconnect', async () => {
                ...
                if (roomId && userId) {
                    ...
                    try {
                        await client_1.prisma.meetingParticipant.updateMany({
    ...
    handleRoomEvents(socket) {
        ...
        socket.on('join-room', async ({ roomId, userId }, callback) => {
            ...
            try {
                ...
                let meeting = await client_1.prisma.meeting.findUnique({ where: { code: roomId } });
                if (!meeting) {
                    ...
                    meeting = await client_1.prisma.meeting.create({
                        data: {
                            code: roomId,
                            creatorId: userId,
                        }
                ...
                const participant = await client_1.prisma.meetingParticipant.upsert({
                    where: {
                        meetingId_userId: {
        ...
        socket.on('leave-room', async ({ roomId, userId }) => {
            ...
            // DB Update
            try {
                const meeting = await client_1.prisma.meeting.findUnique({ where: { code: roomId } });
                if (meeting) {
                    await client_1.prisma.meetingParticipant.updateMany({
    ...
    handleSignalingEvents(socket) {
        ...
        socket.on('toggle-media', ({ roomId, kind, status, userImage }) => {
